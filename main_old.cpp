...
// Cylinder wireframe
void DrawCylinderWireframe(float radius, float height, int slices){
    ...
    for(int i = 0; i < slices; i++){
        ...
        glBegin(GL_LINES);
        glVertex3f(0, 0, 0);
        glVertex3f(radius*cosf(u), radius*sinf(u), 0);
        glEnd();
    }
    ...
    for(int i = 0; i < slices; i++){
        ...
        glVertex3f(radius*cosf(u), radius*sinf(u), height);
        glEnd();
    }
...
void DrawMushroom(float x,float y,float z){
    ...
    if(wireframeMode) DrawCylinderWireframe(0.2f,1.0f,18);
    ...
    // else              DrawEllipsoid(0.6f,0.3f,0.6f,35,18);
...
// ---------------- Cone ----------------
// Point3D ConePoint(float u, float radius, float height){
//     return { radius*cosf(u), radius*sinf(u), 0.0f };
// }

// void DrawConeWireframe(float radius, float height, int slices, int stacks){
//     float du = 2*M_PI/slices;
//     float dv = 1.0f/stacks;
...
//         for(int i=0;i<slices;i++){
//             float u=i*du;
//             Point3D p0 = ConePoint(u,radius*(1-v),0);
//             Point3D p1 = {0,0,height};
//             Point3D p = { p0.x*(1-v)+p1.x*v, p0.y*(1-v)+p1.y*v, p0.z*(1-v)+p1.z*v };
...
//         glBegin(GL_LINES);
//         glVertex3f(0,0,height);                
//         Point3D p = ConePoint(u,radius,0);      
//         glVertex3f(p.x,p.y,p.z);
//         glEnd();
//     }
...
//         for(int j=0;j<=stacks;j++){
//             float v=j*dv;
//             Point3D b0 = ConePoint(u0, radius*(1-v),0);
//             Point3D b1 = ConePoint(u1, radius*(1-v),0);
//             Point3D t = {0,0,height};
//             glColor3f(1,0,0); 
